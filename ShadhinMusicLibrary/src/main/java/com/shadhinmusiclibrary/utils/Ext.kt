...
internal fun isRobiOrArtel(phoneNumber:String) =  Regex( "\\+?(88)?01[68]\\d{8}").matches(phoneNumber)
...
internal fun rangeMap(x: Float, in_min: Float, in_max: Float, out_min: Float, out_max: Float): Float {
...
internal fun <T>CoroutineScope.processWithCoroutine(inputFunc:()->T,callbackFunc:(T)->Unit){
    this.launch {
...
internal fun Date.calender():Calendar = Calendar.getInstance(Locale.getDefault()).apply {time = this@calender }
